...
<section class="expert-problems-full relative mb-14 min-w-0 sm:mb-20 lg:mb-28" data-page-section-type="{{ $section->section_type }}">
    <div class="relative z-10 mx-auto max-w-[98rem] px-0">
        @if($h !== '')
            <h2 class="expert-section-title max-w-3xl text-balance text-[clamp(1.75rem,4vw,2.75rem)] font-bold leading-[1.15] tracking-tight text-white sm:text-4xl">{{ $h }}</h2>
        @endif
        @if($lead !== '')
            <p class="mt-5 max-w-3xl text-[15px] font-normal leading-relaxed text-silver/85 sm:mt-6 sm:text-lg">{{ $lead }}</p>
        @endif
        @if($accent !== '')
        ...
        <div class="{{ $hasHeader ? 'mt-8' : '' }} grid min-w-0 gap-4 sm:grid-cols-2 sm:gap-5 xl:grid-cols-3 xl:gap-6">
            @foreach($items as $item)
                ...
                <article class="expert-problem-card relative flex min-h-0 min-w-0 flex-col rounded-[1.35rem] border p-5 transition-all duration-300 hover:-translate-y-0.5 sm:rounded-[1.5rem] sm:p-7 sm:hover:-translate-y-1 {{ $featured ? 'border-moto-amber/25 bg-gradient-to-br from-moto-amber/[0.05] to-transparent shadow-[0_18px_44px_-14px_rgba(0,0,0,0.55)] ring-1 ring-inset ring-white/[0.04]' : 'border-white/[0.04] bg-white/[0.015] hover:bg-white/[0.03]' }}">
                    ...
                    @endif
                    <h3 class="text-[1.05rem] font-bold leading-snug text-white/95 sm:text-lg md:text-xl">{{ $item['title'] ?? '' }}</h3>
                    @if(filled($item['description'] ?? ''))
                        <p class="mt-3 text-[14px] leading-[1.65] text-silver/85">{{ $item['description'] }}</p>
                    @endif
                    @if(filled($item['solution'] ?? ''))
                        <p class="mt-3 border-t border-white/[0.08] pt-3 text-[13px] font-medium leading-relaxed text-silver/90 sm:text-sm">{{ $item['solution'] }}</p>
                    @endif
                    @if($linkUrl !== '')
        ...
        @if($fn !== '')
            <p class="mt-8 max-w-3xl text-sm font-medium leading-relaxed text-moto-amber/80">{{ $fn }}</p>
        @endif
    </div>
</section>
@else
    ...
    <div class="relative px-0 sm:px-0">
        <div class="relative z-10 mx-auto grid max-w-[98rem] gap-8 sm:gap-10 lg:grid-cols-[1fr_1.5fr] lg:gap-16 xl:grid-cols-[1fr_1.8fr] xl:gap-24">
            ...
            <div class="flex min-w-0 flex-col lg:sticky lg:top-28 lg:max-h-[calc(100vh-8rem)] lg:pt-4">
                @if($h !== '')
                    <h2 class="expert-section-title max-w-2xl text-balance text-[clamp(1.75rem,4vw,2.75rem)] font-bold leading-[1.15] tracking-tight text-white sm:text-4xl">{{ $h }}</h2>
                    @if($lead !== '')
                        <p class="mt-5 max-w-xl text-[15px] font-normal leading-relaxed text-silver/85 sm:mt-6 sm:text-lg">{{ $lead }}</p>
                    @else
                        <p class="mt-5 max-w-xl text-[15px] font-normal leading-relaxed text-silver/85 sm:mt-6 sm:text-lg">Каждый запрос разбираем на практике — без шаблонных сценариев и лишнего давления.</p>
                    @endif
                @endif
                ...
                @if($fn !== '')
                    <p class="mt-8 max-w-lg text-sm font-medium leading-relaxed text-moto-amber/80 lg:mt-auto">{{ $fn }}</p>
                @endif
            </div>
                    ...
                    >
                        ...
                        @if(filled($item['solution'] ?? ''))
                            <div class="mt-auto pt-5 sm:pt-6">
                                <p class="border-t border-white/[0.08] pt-3 text-[13px] font-semibold leading-relaxed text-moto-amber/95 sm:pt-4 sm:text-sm">{{ $item['solution'] }}</p>
                            </div>
                        @endif
                        @if($linkUrl !== '')
