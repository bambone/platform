...
<section x-data="heroVideo({ heroHasVideo: @js($hasHeroVideo) })"
    ...
    <div x-show="videoPlaying"
        ...
        <div class="absolute bottom-6 sm:bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-3 px-5 py-3 rounded-2xl bg-black/70 backdrop-blur-xl border border-white/15 z-[100] pb-[max(1rem,env(safe-area-inset-bottom))]">
            ...
            <label for="hero-video-volume" class="sr-only">Громкость видео</label>
            <input id="hero-video-volume" name="hero_video_volume" type="range" min="0" max="1" step="0.01" x-model="volume" @input="setVolume($event)" class="w-20 accent-moto-amber h-1.5 rounded-full cursor-pointer">
