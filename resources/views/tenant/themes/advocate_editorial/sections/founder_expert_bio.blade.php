@php
    ...
    if ($portraitAlt === '') {
        $portraitAlt = function_exists('tenant') && tenant()?->theme_key === 'advocate_editorial'
            ? ($h !== '' ? $h : 'Дементьев Никита Владимирович')
            : ($h !== '' ? $h : 'Портрет инструктора');
    }
    $trustPoints = [];
...
<section @if($sectionId !== '') id="{{ e($sectionId) }}" @endif class="expert-bio-mega relative mb-14 min-w-0 scroll-mt-24 sm:mb-20 sm:scroll-mt-28 lg:mb-28">
    <div class="relative overflow-hidden rounded-[1.5rem] border border-white/[0.08] bg-gradient-to-br from-[#0c0f17] to-[#050608] shadow-[0_28px_64px_-20px_rgba(0,0,0,0.72)] ring-1 ring-inset ring-white/[0.04] sm:rounded-[2rem]">
        <div class="pointer-events-none absolute left-1/2 top-0 h-[min(28rem,90vw)] w-[min(28rem,90vw)] -translate-x-1/2 rounded-full bg-white/[0.03] blur-[100px]" aria-hidden="true"></div>
        ...
        <div class="relative z-10 grid gap-8 p-6 sm:gap-12 sm:p-10 lg:grid-cols-12 lg:items-center lg:gap-16 lg:p-16 xl:p-20">
            ...
            <div class="relative min-w-0 lg:col-span-5">
                    <div class="pointer-events-none absolute -inset-3 rounded-[1.5rem] bg-gradient-to-br from-white/[0.06] to-transparent blur-2xl sm:-inset-4 sm:rounded-[2rem]"></div>
